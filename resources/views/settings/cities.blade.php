                        ...
                        <div class="card-body">
                            <table id="basic-datatable" class="table dt-responsive nowrap w-100 branchTable table-striped">
                              ...
                              @foreach($citiesData as $key => $arr)
                                    <tr>
                                        <td>{{++$key}}</td>
                                        <td>{{getUserNameById('lms_states','stateID',$arr->stateID,'stateName')}}</td>
                                        <td>{{$arr->cityName}}</td>
                                        ...
                                        <td>
                                            <div>
                                                <input type="checkbox" id="switch{{$arr->cityID}}" checked data-switch="success" data-id="{{$arr->cityID}}" value="0" class="status-switch">
                                                <label for="switch{{$arr->cityID}}" data-on-label="Yes" data-off-label="No" class="mb-0 d-block"></label>
                                            </div>
                                        </td>
                                        @else
                                        <td>
                                            <div>
                                                <input type="checkbox" id="switch{{$arr->cityID}}" data-switch="success" data-id="{{$arr->cityID}}" value="1" class="status-switch">
                                                <label for="switch{{$arr->cityID}}" data-on-label="Yes" data-off-label="No" class="mb-0 d-block"></label>
                                            </div>
                                        </td>
                                        @endif
                                        <td>
                                            <a href="" class="text-info designationsEditBtn" data-bs-toggle="tooltip" data-bs-placement="top"
                                                data-bs-custom-class="info-tooltip" data-bs-title="Edit" style="font-size: 18px;" data-id="{{$arr->cityID}}">
                                                <i class='mdi mdi-pencil'></i>
                                            </a> 
                                         @if(isSuperAdmin())        
                                            <a href="" class="text-danger designationsDeleteBtn" data-bs-toggle="tooltip" data-bs-placement="top"
                                                data-bs-custom-class="danger-tooltip" data-bs-title="Delete" style="font-size: 18px;margin-left: 5px;" data-id="{{$arr->cityID}}">
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header bg-primary text-white">
                        <h4 class="modal-title" id="primary-header-modalLabel">Cities Details</h4>
                    ...
                    <div class="modal-body">
                       <form method="post" id="designationsAddEditModalForm">
                        @csrf
                        <div class="mb-3">
                            <label for="recipient-name" class="col-form-label">States:</label>
                             <select class="form-control" name="stateID" id="stateID">
                                <option value="">Select States</option>
                                @foreach($statesData as $arr)
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="cityName" id="cityName" placeholder="Enter designations name">
                            <span class="cityNameErr"></span>
                        </div>
                        <div class="mb-3">
       ...
       $(document).ready(function() {
        ...
        $(document).on('click','#addNewBtn',function(){
          ...
          $(document).on('submit','#designationsAddEditModalForm',function(event) {
                ...
                var id = $('#id').val();
                var url = (id !== '') ? '{{ route('citiesUpdate') }}' : '{{ route('citiesAdd') }}';
                $.ajax({
                   url: url,
                   type: "POST",
                   dataType:'json',
                   ...
                   success: function(data) {
                    ...
                    // Process the response data here
                        if(data.response=='success'){
                            ...
                            $('#cityName').val(data.values[0].cityName);
                            $('#id').val(data.values[0].cityID);
