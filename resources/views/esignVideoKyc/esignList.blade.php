                  ...
                  <div class="col-md-9">
                     <form action="" method="get">
                        ...
                        @endif
                           @if(!empty($filter))
                           <div class="col-md-1">
                              <a href="kyc/e-sign" class="btn btn-outline-primary" data-toggle="tooltip" data-original-title="success">
                 ...
                 <thead>
                    ...
                    <tbody>
                       ...
                       <tr style="font-size:15px;">
                          ...
                          <td>{{$arr->docRequestByName}}</td>
                          <td>{{ucwords($arr->status)}}</td>
                        @if($arr->loanStatus == 'Rejected')
                            <td>
                                <div>
                                    <input type="checkbox" id="switch{{$arr->esignID}}" checked 
                                        data-switch="success" 
                                        data-esignID="{{$arr->esignID}}" 
                                        value="Approved" 
                                        class="status-switch" 
                                        @if(!(isSuperAdmin() || role() == 'CRM Support') || $arr->leadStatus != 'Approved') disabled @endif>
                                    <label for="switch{{$arr->esignID}}" 
                                           data-on-label="Yes" 
                                           data-off-label="No" 
                                           class="mb-0 d-block">    
                                    </label>
                        ...
                        @else
                            <td>
                                <div>
                                    <input type="checkbox" id="switch{{$arr->esignID}}" 
                                        data-switch="success" 
                                        data-esignID="{{$arr->esignID}}" 
                                        value="Rejected" 
                                        class="status-switch" 
    ...
    $('.exportSelect').change(function() {
        ...
        updateContainers(selectedOption);
         ...
         $(document).on('change','.status-switch',function() {
                var status = $(this).val();
                var esignID = $(this).attr('data-esignID');
                $.ajax({
                   url: "{{ route('esignKycResendUpdate') }}",
                   type: "POST",
                   dataType:'json',
                   data: {
                     _token: "{{ csrf_token() }}",
