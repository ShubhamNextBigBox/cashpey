                  ...
                  <div class="col-md-8">
                     <form action="" method="get">
                        <div class="row">
                           ...
                           <div class="input-group">
                              ...
                              <select class="form-select exportSelect" name="filter">
                                 ...
                                 @endif
                                 @if(rolesAccess('Red Flag Customers', 'export') || isSuperAdmin() || isAdmin())
                        ...
                        @endif
                           @if(!empty($filter))
                           <div class="col-md-1">
                              <a href="redflag/list" class="btn btn-outline-primary" data-toggle="tooltip" data-original-title="success">
               ...
               <table  class="table w-100 table-striped" style="white-space: nowrap;">
                  ...
                  <tbody>
                     ...
                     <tr style="font-size:15px;">
                        ...
                        <td>{{$arr->mobile}}</td>
                        <td>{{$arr->pancard}}</td>
                         @if($arr->redFlagApproved == '1')
                            <td>
                                <div>
                                    <input type="checkbox" id="switch{{$arr->contactID}}" checked 
                                        data-switch="success" 
                                        data-contactID="{{$arr->contactID}}" 
                                        value="0" 
                                        class="status-switch" 
                                        @if(!isSuperAdmin() && !isAdmin()) disabled @endif>
                                    <label for="switch{{$arr->contactID}}" 
                                           data-on-label="Yes" 
                                           data-off-label="No" 
                        ...
                        @else
                            <td>
                                <div>
                                    <input type="checkbox" id="switch{{$arr->contactID}}" 
                                        data-switch="success" 
                                        data-contactID="{{$arr->contactID}}" 
                                        value="1" 
                                        class="status-switch" 
                        ...
                        <td>{{$arr->remarks ?? 'N/A'}}</td>
                        <td>{{df($arr->redFlagAddedOn)}}</td>
                     </tr>
                     @endforeach
                     @endif
                  </tbody>
      ...
      <div class="modal-content">
         <div class="modal-body p-4">
            <div class="text-center">
               ...
               <p class="mt-3"></p>
               <input type="hidden" id="contactIDDelete">
       ...
       $(document).ready(function() {
        ...
        $('.open-modal').click(function(event) {
         ...
         $(document).on('change','.status-switch',function() {
                ...
                $.ajax({
                   url: "{{ route('redFlagLoanApprovalStatusUpdate') }}",
                   type: "POST",
                   dataType:'json',
                   data: {
                     _token: "{{ csrf_token() }}",
